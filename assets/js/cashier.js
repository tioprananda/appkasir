...
// tampilkan nama produk kedalam menu kasir
getProductName = () => {
...
openSales = () => {
    ...
    $('#btn-new-sales').prop('disable', true)
...
// tampilkan code product dalam menu kasir
getCodeByName = () => {
            ...
            if(row.length < 1 ) {
                ...
                db.all(`select * from sales where product_code = '${prd_code}' and invoice_number = '${sales_number}'`, (err, row) => {
                    ...
                    if (row.length < 1) {
                        ...
                        // ketika tombol tambh di klik
                        let messageBox = dialog.showMessageBoxSync({
                            type: `question`,
                            title: `Tambah data`,
                            buttons: [`Yes`, `No`],
                            defaultId: [0, 1],
                            message: `Apa anda yakin ingin menambah data ${product_code}?`,
                          });
                          if(messageBox === 0){
                            // input sales admin ke form dan ke database sales
                            

                            // input order
                            db.run(`insert into sales (input_date, invoice_number, buyer, buyer_id, payment, description, po_number, due_date, term, sales_admin, product_name, barcode, product_code, cost_of_product, price, qty, unit, discount_percent, discount_money, total) values(datetime('now','localtime'), '${sales_number}', '${buyer}', '${buyer_id}' , '${payment}', '${description}', '${po_number}', '${due_date}', '${term}', '', '${prd_name}', '${prd_barcode}', '${prd_code}', '${prd_cost}', '${prd_price}', '${qty}', '${unit}', '${discount_percent}', '${discount_money}', '${total}')`, err => {
                        ...
                        let net_new_total = new_total - discount_percent - discount_money
                          ...
                          let messageBox = dialog.showMessageBoxSync({
                            ...
                            buttons: [`Yes`, `No`],
                            defaultId: [0, 1],
                            message: `data ${product_code} sudah di input, Apa anda yakin ingin menambah data?`,
                          });
                          if(messageBox === 0){
...
// READ SALES MENAMPILKAN HASIL INPUTAN KE TABLE
loadSales = (sales_num) => {
            ...
            rows.map( row => {
                ...
                tr += `
                    <tr class="text-center">
                        ...
                        <td>${row.unit}</td>
                        <td>${discount_info}</td>
                        <td><span>${numberFormat(row.total)}</span></td>
...
// ketika tombol enter dan  ditekan, di form product code aatau product name 
            ...
            if(row.length < 1) {
                db.all(`select total_tax from sales_tax where invoice_number = '${sales_number}'`, (err, row) => {
                    ...
                    if(row.length < 1) {
                        ...
                        $('#total-and-tax, #info-total-sales').html(`Rp. ${numberFormat(net_total_sales)}`)
...
// function input faktur otomatis
salesNumber = () => {
...
// buat inisiasi load buyer
loadBuyerForm = () => {
    ipcRenderer.send('load:buyer-form')
  }

// terima render select buyer
