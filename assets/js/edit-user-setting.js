...
// fungsi tambah user dan password
submitAdmin = () => {
   ...
   if(firstName == "" || lastName == "" || position == "" || userName == '' || password == ''){
    ...
    db.all(`select * from users where username = '${userName}'`, (err, row) => {
        ...
        if(row.length < 1) {
            ...
            dialog.showErrorBox(`Username exists, Username sudah dipakai oleh user lain, silahkan gunakan username yang berbeda.`)
        }
    })
   }
}  
...
submitEditUsername = (username, id) => {
    ...
    if(newUsername == ''){
        dialog.showMessageBoxSync({
            title : `alert`,
            type : `info`,
            message : `Username baru harus di isi.`
        })
