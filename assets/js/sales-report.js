// function close salesreport
closeSalesReport = () => {
    ipcRenderer.send('close:sales-report')
}
...
loadReport = (reportSpan, startMonth, startYear) => {
    ...
    db.all(query, (err, rows) => {
        ...
        if(rows.length < 1) {
            ...
            $('#data').html(tr)
            $('#net-total-sales').html(`Rp. ${numberFormat(totalNetSales)}`)
            $('#total-cogs').html(`Rp. ${numberFormat(totalCogs)}`)
            $('#total-profit').html(`Rp. ${numberFormat(totalProfit)}`)
        }
    })
}
...
// function memilih data report sesuai tanggal
changeDate =() => {
    let reportSpan = $('#report-span').val()
...
exportPdfSalesReport = (filePath, id, joinIds = false) => {
