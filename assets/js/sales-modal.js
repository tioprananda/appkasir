...
// function edit detail belanja dan print struk
submitUpdate = () => {
    switch(title) {
        case 'discount-final' :
            submitDiscountFinal()
...
// function ketika tombol enter diklik maka akan print struk
...
submitUpdateSales = () => {
    ...
    $('#data').find('.input-barcode').each(function() {
        let input_prd_barcode = $(this).val()
        product_barcode.push(input_prd_barcode)
    })
    ...
    for(let i=0;i<id.length;i++) {
        values.push(`('${id[i]}','${input_date[i]}','${sales_number}','${buyer}', '${buyer_id}', '${payment}', '${description}', '${po_number}', '${due_date}', '${term}', '${sales_admin}', '${product_name[i]}', '${product_code[i]}', '${product_barcode[i]}', '${cost_of_product[i]}', '${price[i]}', '${qty[i]}', '${unit[i]}', '${discount_percent[i]}', '${discount_money[i]}','${total[i]}')`)
    }

    let joinValues = values.join(",")

    let query = `replace into sales(id, input_date, invoice_number, buyer, buyer_id, payment, description, po_number, due_date, term, sales_admin, product_name, product_code, barcode, cost_of_product, price, qty, unit, discount_percent, discount_money, total) values${joinValues}`
